...
import { z } from 'zod';
import { getHookMetadataBySlug } from '@/generated/hooks-metadata';
import { logger } from '@/lib/logger';
import { getDisplayTitle } from '@/lib/utils';

export const alt = 'Claude Pro Directory - Hook';
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const hook = getHookMetadataBySlug(validatedParams.slug);
    ...
    return new ImageResponse(
      ...
      >
        ...
        >
          ...
          >
            {hook.description?.substring(0, 150)}
            {hook.description && hook.description.length > 150 ? '...' : ''}
          </p>
        ...
        >
          Created by {hook.author || 'Community'}
        </div>
      </div>,
      { ...size }
    );
    ...
    logger.error(
      'Hook opengraph validation error',
