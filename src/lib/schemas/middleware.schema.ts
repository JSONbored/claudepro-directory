...
const userAgentSchema = z
  ...
  .describe(
    "Browser user agent string sanitized by removing control characters",
  );
...
const httpContentTypeSchema = z
  ...
  .describe(
    "Valid HTTP content type header normalized to lowercase without charset parameters",
  );
...
export const staticAssetSchema = z
  ...
  .describe(
    "Path to static assets including Next.js resources, images, and common web files",
  );
...
export const middlewareRateLimitConfigSchema = z
  .object({
    windowMs: positiveInt
      ...
      .max(3600000)
      .describe(
        "Time window in milliseconds for rate limiting (1 second to 1 hour)",
      ), // 1 second to 1 hour
...
const arcjetDecisionSchema = z
  .object({
    isDenied: z
      .function()
      .describe("Function to check if the request was denied by Arcjet"),
    isAllowed: z
      .function()
      .describe("Function to check if the request was allowed by Arcjet"),
    ip: z.string().describe("IP address that made the request"),
    reason: z
      .object({
        isRateLimit: z
          .function()
          .describe("Function to check if denial was due to rate limiting"),
        isBot: z
          .function()
          .describe("Function to check if denial was due to bot detection"),
        isShield: z
          .function()
          .describe("Function to check if denial was due to shield protection"),
      })
  ...
  .describe(
    "Arcjet security decision object containing authorization status and denial reasons",
  );
...
export const requestValidationSchema = z
  .object({
    ...
    userAgent: userAgentSchema
      .optional()
      .describe("Browser or client user agent string"),
    ip: ipAddressSchema
      .optional()
      .describe("Client IP address from headers or connection"),
...
export const searchQueryValidationSchema = z
  .object({
    q: z
      ...
      .optional()
      .describe(
        "Search query string with alphanumeric and basic punctuation only (max 500 chars)",
      ),
    category: z
  ...
  .describe(
    "Search query parameters with validation for query string, category, and pagination",
  );
...
export const apiEndpointTypeSchema = z
  ...
  .describe(
    "Classification of API endpoint type for rate limiting and security policies",
  );
...
const middlewareResponseSchema = z
  .object({
    status: positiveInt
      .min(100)
      .max(599)
      .describe("HTTP status code for the response (100-599)"),
    headers: z
      .record(
        z.string().describe("Header name"),
        z.string().describe("Header value"),
      )
      .describe("Response headers as key-value pairs"),
    body: z.string().optional().describe("Optional response body content"),
  })
  .describe(
    "HTTP response structure for middleware including status, headers, and body",
  );
...
const securityHeadersSchema = z
  .object({
    ...
    "x-frame-options": z
      .string()
      .optional()
      .describe("Header to prevent clickjacking attacks"),
  ...
  .describe(
    "Collection of security-related HTTP headers for web application hardening",
  );
...
export type MiddlewareRateLimitConfig = z.infer<
  typeof middlewareRateLimitConfigSchema
>;
