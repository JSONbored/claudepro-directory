...
export const errorSeveritySchema = z
  ...
  .describe(
    "Categorizes the severity level of an error for prioritization and alerting",
  );
...
export const errorTypeSchema = z
  ...
  .describe(
    "Classifies errors by their root cause for proper handling and routing",
  );
...
export const errorContextSchema = z
  .record(
    z
      .string()
      .max(50, "Context key too long")
      .describe("Context key identifier"),
    z
      .union([
        z
          .string()
          .max(ERROR_LIMITS.MAX_CONTEXT_VALUE_LENGTH)
          .describe("String context value"),
  ...
  .describe(
    "Additional metadata and diagnostic information associated with an error",
  );
...
export const errorInputSchema = z
  ...
  .describe(
    "Flexible error input accepting Error objects, strings, or error-like objects",
  );
...
export const sanitizedErrorSchema = z
  .object({
    error: nonEmptyString
      .max(200)
      .describe("Short error identifier or category name"),
    message: z
      .string()
      .max(200)
      .describe("User-facing error message, sanitized for security"),
    code: z
      .string()
      .regex(/^[A-Z]{3}_[A-Z0-9]{8,12}$/, "Invalid error code format")
      .describe(
        "Unique error code for tracking and debugging (e.g., ERR_VALIDATION01)",
      ),
  ...
  .describe(
    "Production-safe error response with sanitized fields and tracking metadata",
  );
...
const errorValidationResultSchema = z
    ...
    z
      .object({
        ...
        fallback: z
          .never()
          .optional()
          .describe("Not used for valid Error objects"),
      })
  ...
  .describe(
    "Type-safe discriminated union representing error validation outcomes",
  );
...
export function validateErrorContext(
  ...
  try {
      ...
      if (typeof key === "string" && key.length <= 50) {
        if (typeof value === "string") {
          sanitized[key] = value.slice(
            0,
            ERROR_LIMITS.MAX_CONTEXT_VALUE_LENGTH,
          );
    ...
    logger.error(
      ...
      {
        error:
          error instanceof z.ZodError ? error.issues.join(", ") : String(error),
...
export function determineErrorType(
  error: unknown,
): z.infer<typeof errorTypeSchema> {
...
const errorHandlerConfigSchema = z
  .object({
    ...
    operation: z
      ...
      .max(100)
      .optional()
      .describe("Operation or function name being executed"),
    method: z
      .string()
      .max(10)
      .optional()
    ...
    sanitizeResponse: z
      ...
      .optional()
      .describe(
        "Whether to sanitize error response to remove sensitive information",
      ),
  ...
  .describe(
    "Configuration options for error handling, logging, and response formatting",
  );
...
const errorResponseSchema = z
  .object({
    ...
    error: nonEmptyString
      .max(200)
      .describe("Error type or category identifier"),
    message: nonEmptyString
      .max(1000)
      .describe("Detailed error message for the client"),
    ...
    requestId: requestIdSchema
      .optional()
      .describe("Request identifier for tracing"),
    details: z
      .array(
        z
          .object({
            ...
            message: z
              .string()
              .max(500)
              .describe("Specific error message for this detail"),
            code: z
              .string()
              .max(50)
              .optional()
              .describe("Error code specific to this detail"),
          })
          .describe("Individual error detail item"),
      )
      .optional()
      .describe(
        "Array of granular error details, typically for validation errors",
      ),
    stack: z
      .string()
      .max(ERROR_LIMITS.MAX_STACK_TRACE_LENGTH)
  ...
  .describe(
    "Standardized API error response structure with comprehensive metadata",
  );
