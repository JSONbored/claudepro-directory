...
import { z } from 'zod';
import { cacheKeyString, KEY_LIMITS } from '@/src/lib/schemas/primitives/api-cache-primitives';
import { nonNegativeInt, positiveInt, viewCount } from '@/src/lib/schemas/primitives/base-numbers';
...
export const cacheKeyParamsSchema = z
  .object({
    category: cacheCategorySchema.describe('Content category for cache key construction'),
...
export const popularItemsQuerySchema = z
  .object({
    category: cacheCategorySchema.describe('Category to retrieve popular items from'),
    limit: positiveInt
      .max(100)
      .default(10)
  ...
  .describe('Query parameters for retrieving popular items from cache with pagination support');
...
export const cacheStatsSchema = z
  .object({
    totalViews: nonNegativeInt.describe('Total view count across all cached content'),
    totalCopies: nonNegativeInt.describe('Total number of copy operations tracked'),
    topCategories: z
      .array(
        z
          .object({
    ...
    memoryUsage: nonNegativeInt.optional().describe('Current memory usage in bytes (optional)'),
  })
  .describe('Aggregated cache statistics including views, copies, and performance metrics');
...
const redisZRangeResponseSchema = z
  ...
  .describe('Raw Redis ZRANGE response array containing alternating member-score pairs');
...
const popularItemWithScoreSchema = z
  .object({
    ...
    views: viewCount.max(CACHE_LIMITS.MAX_VIEW_COUNT).describe('Total view count for this item'),
  })
...
export const cacheInvalidationResultSchema = z
  .object({
    ...
    keysDeleted: nonNegativeInt.describe('Number of cache keys successfully deleted'),
  ...
  .describe('Results of a cache invalidation operation with performance metrics');
...
const rateLimitTrackingSchema = z
  .object({
    ...
    timeUntilReset: z.number().min(0).describe('Seconds until rate limit window resets'),
    isNearLimit: z.boolean().describe('Whether current usage is approaching the rate limit'),
...
const redisConnectionStatusSchema = z
  .object({
    ...
    lastConnectionAttempt: z
      .date()
      .nullable()
      .describe('Timestamp of last connection attempt (null if never attempted)'),
    consecutiveFailures: nonNegativeInt.describe('Number of consecutive connection failures'),
    totalOperations: nonNegativeInt.describe('Total number of cache operations performed'),
    failedOperations: nonNegativeInt.describe('Number of failed cache operations'),
  })
