...
const homePagePropsSchema = z
    ...
    ...Object.fromEntries(
      HOMEPAGE_FEATURED_CATEGORIES.map((category) => [
        ...
        z
          ...
          .describe(`Collection of ${category} content items`),
      ]),
    ),
    allConfigs: z
      .array(unifiedContentItemSchema)
  ...
  .describe(
    "Props for the home page containing categorized content collections",
  );
...
const homePageClientPropsSchema = z
  .object({
    initialData: homePagePropsSchema.describe(
      "Initial server-side data for client hydration",
    ),
    stats: z
      .object({
  ...
  .describe(
    "Props for client-side home page component with SSR hydration data",
  );
