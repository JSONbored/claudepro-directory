...
export const pageSchema = z.coerce.number().int().min(1).max(10000).default(1).openapi({
  description: 'Page number for pagination (1-based)',
  example: 1,
});

export const limitSchema = z.coerce.number().int().min(1).max(1000).default(50).openapi({
...
export const searchQueryParamsSchema = z.object({
  ...
  page: pageSchema,
  limit: limitSchema,
  sortBy: z.enum(['relevance', 'date', 'name', 'popularity']).default('relevance').openapi({
...
export const trendingQuerySchema = z.object({
  algorithm: z.enum(['trending', 'popular', 'recent']).optional().default('trending').openapi({
    description: 'Trending algorithm to use',
    example: 'trending',
  }),
  period: z.enum(['day', 'week', 'month', 'all']).optional().default('week').openapi({
...
export const contentListDataSchema = z.object({
  items: z.array(z.unknown()).openapi({ description: 'Array of content items' }),
  total: z.number().int().min(0).openapi({ description: 'Total number of items', example: 42 }),
  page: pageSchema,
  limit: limitSchema,
  category: contentTypeSchema,
});
...
export const searchResultsDataSchema = z.object({
  results: z.array(z.unknown()).openapi({ description: 'Array of search results' }),
  total: z.number().int().min(0).openapi({ description: 'Total matching results', example: 15 }),
  page: pageSchema,
  limit: limitSchema,
  query: searchQuerySchema,
  filters: z.object({}).passthrough().optional().openapi({ description: 'Applied filters' }),
});

export const trendingItemSchema = z.object({
  category: contentTypeSchema,
  slug: slugSchema,
  title: z.string().openapi({ description: 'Content title', example: 'Awesome Agent' }),
  views: z.number().int().min(0).openapi({ description: 'Total views', example: 1234 }),
  viewsToday: z
    .number()
    .int()
    .min(0)
...
export const trendingDataSchema = z.object({
  trending: z.array(trendingItemSchema).openapi({ description: 'Trending content items' }),
  algorithm: z
...
export const guidesTrendingDataSchema = z.object({
  guides: z.array(trendingGuideItemSchema).openapi({ description: 'Trending guides list' }),
  category: z.string().openapi({ description: 'Category filter applied', example: 'guides' }),
  count: z
    .number()
    .int()
    .min(0)
    ...
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: successResponseSchema.extend({
              data: z.unknown().openapi({ description: 'Content item with full details' }),
            }),
          },
        },
      },
