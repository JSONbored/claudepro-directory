...
const agentConfig: ContentTypeConfigRegistry["agents"] = {
  ...
  generators: {
    installation: (item) => ({
      ...
      requirements: [
        "Claude Code or Claude SDK",
        "Access to specified tools and functions",
      ],
    }),
...
const commandConfig: ContentTypeConfigRegistry["commands"] = {
  ...
  generators: {
    installation: (item) => ({
      ...
      requirements: [
        "Claude Code CLI",
        "Command-specific dependencies (see content below)",
      ],
    }),
      ...
      return [
        {
          ...
          solution:
            "Verify script has executable permissions (chmod +x) and correct shebang line.",
        },
        {
          issue: "Hook dependencies not found",
          solution:
            "Install required tools (jq, prettier, etc.) and ensure they are in your PATH.",
        },
      ];
    },
  },
      ...
      if (
        ...
        if (config?.colorScheme) {
          detectedRequirements.push(
            "Terminal with color support (256 colors or truecolor)",
          );
        }
      }
...
export function getContentTypeConfig(
  ...
  const normalizedCategory =
    category.toLowerCase() as keyof ContentTypeConfigRegistry;
