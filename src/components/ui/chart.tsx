...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Generate CSS rules as style elements using React components
  return (
    <>
      {Object.entries(THEMES).map(([theme, prefix]) => {
        ...
        return <style key={theme}>{`${selector} { ${cssVars} }`}</style>;
      })}
    </>
  );
};
...
const ChartTooltipContent = ({
  ...
  labelFormatter?: (value: unknown, payload: Array<TooltipPayload>) => React.ReactNode;
  formatter?: (
    value: unknown,
    name: unknown,
    item: TooltipPayload,
