...
import { Thermometer } from "@/src/lib/icons";
...
export function SidebarDetailsCard({
  ...
  const hasConfiguration =
    "configuration" in item && typeof item.configuration === "object";
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {item.category && (
          <div>
            <h4 className={`${UI_CLASSES.FONT_MEDIUM} ${UI_CLASSES.MB_1}`}>
              Category
            </h4>
            <Badge
              variant="default"
        ...
        {hasConfiguration &&
          ...
          typeof (item.configuration as { temperature?: number })
            .temperature === "number" && (
            <div>
              <h4 className={`${UI_CLASSES.FONT_MEDIUM} ${UI_CLASSES.MB_1}`}>
                Temperature
              ...
              <div className={UI_CLASSES.FLEX_ITEMS_CENTER_GAP_2}>
                ...
                >
                  {String(
                    (item.configuration as { temperature: number }).temperature,
                  )}
                </Badge>
              </div>
            </div>
        ...
        {hasAuth && (
          <div>
            ...
            <p className={UI_CLASSES.TEXT_SM_MUTED}>
              {(item as { requiresAuth: boolean }).requiresAuth
                ? "Required"
                : "Not required"}
            </p>
          </div>
