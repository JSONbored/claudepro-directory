...
export type GuideMetadata = Pick<GuideContent, 'slug' | 'title' | 'seoTitle' | 'description' | 'author' | 'tags' | 'category' | 'subcategory' | 'dateAdded' | 'source' | 'keywords' | 'difficulty' | 'readingTime'>;
...
export const guidesMetadataBySlug = new Map(guidesMetadata.map(item => [item.slug, item]));

export function getGuideMetadataBySlug(slug: string): GuideMetadata | null {
  return guidesMetadataBySlug.get(slug) || null;
}
