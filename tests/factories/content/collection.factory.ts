...
export const collectionFactory = Factory.define<CollectionContent, CollectionFactoryTransientParams>(
  ({ sequence, transientParams }) => {
    ...
    const slug = faker.helpers.slugify(collectionName).toLowerCase() + (sequence > 1 ? `-${sequence}` : '');
    ...
    return {
      ...
      items: Array.from({ length: itemCount }, () => ({
        category: faker.helpers.arrayElement(['agents', 'mcp', 'commands', 'rules', 'hooks', 'statuslines'] as const),
