...
export default {
  rules: {
    ...
    // ============================================================================
    // Enforces design system usage and warns on inline Tailwind classes
    'design-system-no-inline-tailwind': {
      meta: {
        type: 'suggestion',
        docs: {
          description: 'Prefer design system utilities over inline Tailwind classes',
          category: 'Best Practices',
          recommended: true,
        },
        fixable: null, // Manual fixes only - too complex for auto-fix
        schema: [
          {
            type: 'object',
            properties: {
              allowedPatterns: {
                type: 'array',
                items: { type: 'string' },
                description: 'Regex patterns for allowed inline Tailwind classes',
              },
            },
      ...
      create(context) {
        ...
        const tailwindPatterns = [
          // Spacing
          { pattern: /\bmb-(\d+|auto|px|py)\b/, utility: 'marginBottom.*', category: 'spacing' },
          { pattern: /\bmt-(\d+|auto|px|py)\b/, utility: 'marginTop.*', category: 'spacing' },
          { pattern: /\bspace-y-(\d+)\b/, utility: 'spaceY.*', category: 'spacing' },
          ...
          for (const { pattern, utility, category } of tailwindPatterns) {
            ...
            if (match) {
              ...
              if (!hasDesignSystemImport) {
                ...
                // Design system is imported, but inline Tailwind is still used
                context.report({
                  node,
                  messageId: 'inlineTailwindDetected',
                  data: {
                    className: match[0],
                  },
                });
            ...
            if (node.name.name === 'className') {
              if (node.value.type === 'Literal' && typeof node.value.value === 'string') {
                ...
                if (expr.type === 'TemplateLiteral') {
                  // Template literal: className={`${var} mb-4`}
                  const templateText = sourceCode.getText(expr);
                  checkClassName(node, templateText);
    ...
    'design-system-prefer-design-system-utility': {
      meta: {
        type: 'suggestion',
        docs: {
          description: 'Suggest design system utilities for common patterns',
          category: 'Best Practices',
          recommended: true,
        },
        fixable: null,
      ...
      create(context) {
        ...
        // Common patterns that have design system equivalents
        const suggestions = [
          { pattern: /\bflex\s+flex-col\s+gap-4\b/, utility: 'stack.comfortable' },
          { pattern: /\bflex\s+items-center\s+gap-2\b/, utility: 'cluster.compact' },
          { pattern: /\bmb-4\b/, utility: 'marginBottom.default' },
          { pattern: /\bmt-4\b/, utility: 'marginTop.default' },
          { pattern: /\btext-muted-foreground\b/, utility: 'muted.default' },
          { pattern: /\bh-4\s+w-4\b/, utility: 'iconSize.sm' },
          { pattern: /\brounded-lg\b/, utility: 'radius.lg' },
        ];

        function suggestDesignSystem(node, classNameValue) {
          if (!classNameValue || typeof classNameValue !== 'string') return;

          for (const { pattern, utility } of suggestions) {
            if (pattern.test(classNameValue)) {
              context.report({
                node,
                messageId: 'suggestUtility',
                data: {
                  pattern: classNameValue,
                  utility,
                },
              });
              break; // Only suggest one utility per node
            }
          }
        }
        ...
        return {
          JSXAttribute(node) {
            if (node.name.name === 'className') {
              if (node.value.type === 'Literal' && typeof node.value.value === 'string') {
                suggestDesignSystem(node, node.value.value);
              } else if (node.value.type === 'JSXExpressionContainer') {
                const expr = node.value.expression;
                if (expr.type === 'TemplateLiteral') {
                  const templateText = sourceCode.getText(expr);
                  suggestDesignSystem(node, templateText);
                } else if (expr.type === 'Literal' && typeof expr.value === 'string') {
                  suggestDesignSystem(node, expr.value);
                }
              }
            }
          },
