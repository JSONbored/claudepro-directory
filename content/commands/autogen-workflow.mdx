...
category: commands
description: >-
  Orchestrate multi-agent workflows using Microsoft AutoGen v0.4 with role-based
  task delegation, conversation patterns, and collaborative problem solving
cardDescription: >-
  Orchestrate multi-agent workflows using Microsoft AutoGen v0.4 with role-based
  task delegation, conversation patterns, and collaborative...
...
usageSnippet: '/autogen-workflow [options] <workflow_description>'
copySnippet: '/autogen-workflow [options] <workflow_description>'
scriptLanguage: text
scriptBody: >-
  The `/autogen-workflow` command creates sophisticated multi-agent workflows
  using Microsoft AutoGen v0.4's actor model with role-based delegation and
  ...
  - `--conversation-pattern=<type>` - Conversation pattern (sequential,
  round-robin, hierarchical)
  ...
  **Command:**
  ...
  /autogen-workflow --coding --agents="architect,coder,reviewer,tester" "Build a
  ...
  **Command:**
  ...
  /autogen-workflow --research --agents="researcher,analyst,synthesizer"
  --tools="web-search" "Analyze the current state of edge computing in 2025"
  ...
  **Command:**
  ...
  /autogen-workflow --review
  --agents="security-auditor,performance-reviewer,style-checker" "Review
  ...
  **Command:**
  ...
  /autogen-workflow --debug --agents="debugger,root-cause-analyst,fixer"
  --human-in-loop "Investigate failing user login tests"
  ...
  **Generated Workflow:**
  ...
  1. **Clear Role Definition**: Each agent should have a specific, well-defined
  role

  2. **Termination Conditions**: Always set clear termination conditions to
  avoid infinite loops

  3. **Tool Access**: Only grant tools to agents that need them

  4. **Human-in-Loop**: Require approval for critical actions (deployments,
  deletions)
...
robotsIndex: true
robotsFollow: true
commandSyntax: '/autogen-workflow [options] <workflow_description>'
---
