...
Check model string extraction: jq -r '.model.id // .model.display_name'. Verify case statement matches model name pattern. Test model matching: echo "$model" | grep -i 'claude-sonnet-4'. Add custom model: Add case for *"your-model"*) context_limit=X ;; before default case. Check model name format in Claude Code JSON output.
