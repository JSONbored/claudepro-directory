...
scriptBody: >-
  ...
  federation. Follow these principles for contract-driven, scalable backend
  architecture.
  ...
  - Define API contracts (OpenAPI, tRPC schemas, GraphQL SDL) BEFORE writing
  code
  ...
  - Generate interactive API docs from schemas (Swagger UI, Scalar, GraphQL
  Playground)
  ...
  Always define API contracts before implementation, use Zod for runtime
  validation with tRPC, follow REST semantics strictly for HTTP APIs, implement
  comprehensive error handling with proper status codes, and generate TypeScript
  types from schemas for end-to-end type safety.
---
