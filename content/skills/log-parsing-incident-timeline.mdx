...
category: skills
description: >-
  Parse web, application, and system logs into structured incidents and
  timelines with anomaly detection. Supports multiple log formats (Apache/Nginx
  access logs, JSON structured logs, syslog, Docker logs, AWS CloudWatch), fast
  pattern matching with ripgrep, timestamp parsing and correlation, error
  aggregation, request tracing across distributed systems, performance analysis
  with percentile calculations, and security audit capabilities. Includes
  PII/secret redaction, multi-service correlation, and timeline generation for
  post-incident analysis.
cardDescription: >-
  Parse web, application, and system logs into structured incidents and
  timelines with anomaly detection.
...
installable: true
installCommand: >-
  curl -L https://heyclau.de/downloads/skills/log-parsing-incident-timeline.zip
  -o log-parsing-incident-timeline.zip && unzip -o
  log-parsing-incident-timeline.zip -d ./log-parsing-incident-timeline
usageSnippet: >-
  Claude can parse application logs, server logs, and system logs to extract
  errors, create incident timelines, identify patterns, and correlate events
  across distributed systems. Transform raw log data into actionable insights.
copySnippet: >-
  # Filter 5xx errors from nginx and sort by time

  rg -n ' 5\d\d ' access.log | awk '{print $4, $5, $9, $7}' | sort >
  timeline.txt
...
prerequisites:
  ...
  - >-
    Log file access permissions - read access to log files in various formats
    (text, JSON, syslog, etc.) and write access for parsed output files
  - >-
    Regular expression knowledge or pattern matching tools (grep, awk, sed) for
