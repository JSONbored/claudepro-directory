...
category: skills
description: >-
  Crawl domains respectfully, extract readable content, dedupe, and generate
  structured summaries. Perfect for research, competitive analysis, and content
  aggregation. This skill enables automated web crawling with robots.txt
  compliance, intelligent content extraction using readability algorithms,
  boilerplate removal, content deduplication, structured data extraction
  (JSON-LD, Schema.org, Open Graph), and multi-format export (Markdown, JSON,
  CSV). Supports both static HTML parsing with BeautifulSoup/Cheerio and dynamic
  JavaScript rendering with Playwright/Puppeteer for modern SPAs. Features
  include rate limiting, user agent rotation, sitemap-based crawling, pagination
  handling, and ethical scraping practices.
cardDescription: >-
  Crawl domains respectfully, extract readable content, dedupe, and generate
...
installable: true
installCommand: >-
  curl -L https://heyclau.de/downloads/skills/website-crawler-summarizer.zip -o
  website-crawler-summarizer.zip && unzip -o website-crawler-summarizer.zip -d
  ./website-crawler-summarizer
usageSnippet: >-
  Claude can crawl websites, extract content from web pages, clean HTML to
  readable text, respect robots.txt, and generate structured summaries or
  documentation from web content. Perfect for research, competitive analysis,
...
prerequisites:
  ...
  - >-
    HTML parsing library (Cheerio for Node.js, BeautifulSoup for Python) for
