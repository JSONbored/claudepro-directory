...
category: skills
description: >-
  Build high-performance async REST APIs with FastAPI, Python's fastest-growing
  web framework. Automatic OpenAPI/Swagger documentation, type-safe validation
  with Pydantic, native async/await support, and dependency injection for clean
  architecture. Includes WebSocket support, background tasks, and
...
prerequisites:
  ...
  - pydantic ^2.0.0
  - >-
    ASGI server (uvicorn, hypercorn, or daphne) for running FastAPI applications
    in production
  - >-
    Database driver for async operations (asyncpg for PostgreSQL, aiomysql for
    MySQL, or motor for MongoDB) when using database integration
tags:
  - fastapi
  - python
  - api
