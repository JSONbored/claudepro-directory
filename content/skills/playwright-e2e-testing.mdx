...
category: skills
description: >-
  Automate end-to-end testing with Playwright - the modern browser automation
  framework that supports Chromium, Firefox, and WebKit with a single API.
  Generate AI-powered tests with GitHub Copilot integration, leverage official
  MCP (Model Context Protocol) support for accessibility-driven interactions,
  and run cross-browser tests with native parallel execution and test sharding.
  Includes visual regression testing with screenshot comparison, API testing and
  contract validation, network request interception and mocking, trace viewer
  for debugging, and CI/CD integration with GitHub Actions, GitLab CI, and Azure
  Pipelines.
cardDescription: >-
  Automate end-to-end testing with Playwright - the modern browser automation
  framework that supports Chromium, Firefox, and WebKit with a...
...
prerequisites:
  ...
  - >-
    Browser binaries installed via Playwright (npx playwright install) for
    target browsers (Chromium, Firefox, WebKit)
  - >-
    Test application or website accessible for testing (local development
    server, staging environment, or production URL)
tags:
  - testing
  - playwright
  - e2e
