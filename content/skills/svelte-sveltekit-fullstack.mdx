...
category: skills
description: >-
  Build full-stack web apps with Svelte and SvelteKit. Minimal runtime overhead,
  reactive components, and server-side rendering. The most admired frontend
  framework of 2025. Svelte compiles components to vanilla JavaScript at build
  time, resulting in zero runtime overhead and exceptional performance.
  SvelteKit adds file-based routing, server-side rendering (SSR), static site
  generation (SSG), API routes, form actions with progressive enhancement, load
  functions for data fetching, hooks for authentication and middleware, and
  adapters for deployment to any platform (Vercel, Netlify, Node.js, Cloudflare,
  etc.). Build reactive dashboards, full-stack applications with authentication
  and databases, static sites with Markdown processing, and real-time
  collaborative tools.
cardDescription: >-
  Build full-stack web apps with Svelte and SvelteKit. Minimal runtime overhead,
  reactive components, and server-side rendering.
...
prerequisites:
  ...
  - vite ^5.0.0
  - >-
    Node.js 18+ runtime environment for running SvelteKit development server and
    build processes
  - >-
    Package manager (npm, pnpm, yarn, or bun) for installing SvelteKit
    dependencies and managing project packages
tags:
  - svelte
  - sveltekit
  - frontend
