...
category: hooks
description: >-
  Automatically updates Jest snapshots when component files are modified
  significantly. This PostToolUse hook provides comprehensive Jest snapshot
  management with intelligent Jest snapshot detection and updating for modified
  components with automatic snapshot file discovery in __snapshots__
  directories, test file detection using pattern matching (.test.*, .spec.*),
  and component-to-test mapping, multi-framework support (React, Vue, Angular,
  vanilla JS) with framework-specific test file detection, snapshot change
  analysis and impact assessment with git diff analysis to detect significant
  changes (configurable threshold), test suite validation before snapshot
  updates with test execution and failure detection, orphaned snapshot cleanup
  and maintenance with automatic orphaned snapshot detection, interactive
  confirmation for significant snapshot changes with change threshold
  configuration, test coverage analysis and reporting with coverage metrics, and
  parallel test execution optimization for large codebases with --maxWorkers
  configuration. The hook automatically detects component files (.jsx, .tsx,
  .js, .ts, .vue, .component.ts), finds related test files and snapshot files,
  analyzes component changes using git diff, runs tests to detect snapshot
  mismatches, updates snapshots when needed using --updateSnapshot flag, and
  provides comprehensive snapshot management reports with actionable
  recommendations.
cardDescription: >-
  Automatically updates Jest snapshots when component files are modified
  significantly.
...
installable: true
installCommand: >-
  mkdir -p .claude/hooks && touch .claude/hooks/jest-snapshot-auto-updater.sh &&
  chmod +x .claude/hooks/jest-snapshot-auto-updater.sh
usageSnippet: >-
  mkdir -p .claude/hooks && touch .claude/hooks/jest-snapshot-auto-updater.sh &&
  chmod +x .claude/hooks/jest-snapshot-auto-updater.sh
copySnippet: |-
...
Old snapshots persist when components renamed. Run cleanup: 'npm test -- --updateSnapshot --clearCache' deleting unused snapshots. Or manually remove from __snapshots__ directory. Verify snapshot cleanup. Test with component renaming.
