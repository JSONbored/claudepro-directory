...
description: >-
  ...
  alerting when error rates exceed configurable thresholds. Features include
  real-time error pattern detection in log files with automatic log file
  discovery across standard and framework-specific locations, configurable error
  rate thresholds and alerting with environment variable configuration,
  multi-log file monitoring (*.log, logs/*, custom paths) with recursive
  directory scanning, error severity classification (fatal, error, warning) with
  pattern-based severity detection, recent error sample display for quick
  debugging with truncated error messages, time-based error rate calculations
  with configurable time windows, framework-specific error pattern recognition
  for Node.js, Python, Next.js, and Docker applications, and silent operation
  with threshold-based notifications. The hook automatically discovers log files
  in common locations and monitors Docker container logs when Docker is
  available, providing comprehensive error monitoring across development
  environments.
...
- Multi-log file monitoring (*.log, logs/*, custom paths) with recursive directory scanning and support for multiple log file formats (*.log, *.out, *.err)
...
Add custom log paths to LOG_DIRS array: LOG_DIRS+=('build/logs' '.next/logs'). Extend file patterns: find with -name '*.out' -o -name 'app*.log' for comprehensive coverage. Check framework-specific log locations (Next.js .next/, Django logs/, Flask app.log). Use recursive find with appropriate depth limits.
...
Use log file modification time to detect log rotation: stat -c %Y logfile. Check for multiple log files with rotation patterns: *.log, *.log.1, *.log.2. Use log file timestamps instead of line counts for time-based calculations. Consider using log rotation detection: check for log file size changes or new log files.
