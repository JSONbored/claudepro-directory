...
description: >-
  ...
  CloudFormation template files (JSON, YAML) and runs comprehensive validation
  including syntax error detection, AWS best practices compliance checking, type
  mismatch validation, resource property validation, and SAM template support.
  Features include advanced CloudFormation template validation with cfn-lint
  supporting multi-region validation, custom rules, and comprehensive error
  reporting. The hook provides fallback to AWS CLI validation when cfn-lint is
  unavailable, ensuring validation works in all environments. Supports both JSON
  and YAML template formats, automatically transforms SAM templates to
  CloudFormation for validation, and integrates seamlessly into development
  workflows and CI/CD pipelines for infrastructure as code quality assurance.
cardDescription: >-
  Validates AWS CloudFormation templates for syntax errors and best practices
...
installCommand: >-
  ...
  && chmod +x .claude/hooks/aws-cloudformation-validator.sh
usageSnippet: >-
  mkdir -p .claude/hooks && touch .claude/hooks/aws-cloudformation-validator.sh
  && chmod +x .claude/hooks/aws-cloudformation-validator.sh
copySnippet: |-
...
scriptBody: >-
  ...
  if [[ "$FILE_PATH" == *.cf.json ]] || [[ "$FILE_PATH" == *.cf.yaml ]] || [[
  "$FILE_PATH" == *cloudformation*.yaml ]] || [[ "$FILE_PATH" ==
  *cloudformation*.json ]]; then
