...
description: >-
  ...
  nesting level detection. This hook runs on file write operations and analyzes
  JavaScript, TypeScript, and Python code to detect maintainability issues
  before they become technical debt. Features include real-time complexity
  monitoring for JavaScript, TypeScript, and Python using pattern matching and
  AST-based analysis, configurable line count thresholds (default: 500 lines)
  with customizable warning and error levels, function count analysis and
  alerting to detect over-modularization or under-modularization, nesting level
  detection for code structure using brace counting and indentation depth
  analysis, instant feedback on code maintainability issues with severity levels
  (INFO, WARNING, ERROR), multi-language support with pattern matching for .js,
  .jsx, .ts, .tsx, and .py files, cyclomatic complexity estimation using control
  flow analysis, and integration with ESLint complexity rules (max-complexity)
  and Radon for Python for comprehensive analysis.
cardDescription: >-
  Alerts when code complexity exceeds thresholds in real-time using cyclomatic
