...
installable: true
installCommand: >-
  mkdir -p .claude/hooks && touch .claude/hooks/vue-composition-api-linter.sh &&
  chmod +x .claude/hooks/vue-composition-api-linter.sh
usageSnippet: >-
  mkdir -p .claude/hooks && touch .claude/hooks/vue-composition-api-linter.sh &&
  chmod +x .claude/hooks/vue-composition-api-linter.sh
copySnippet: |-
  ...
  "vue_composition_api_linter": {
    ...
    "tools": {
      "eslint": {
        ...
        "rules": ["vue/component-api-style", "vue/define-macros-order", "vue/no-ref-as-operand"]
      },
      "vue-tsc": {
...
### Hook incorrectly flags 'this' usage in <template> section as error

grep matches template expressions not just script. Refine: 'grep "<script" -A 200 "$FILE_PATH" | grep "this\."' limiting to script blocks. Or exclude: 'grep -v "<template>" before checking. Verify script block detection. Check Vue component structure.
### defineProps/defineEmits detection fails with TypeScript generic syntax

Pattern 'defineProps(' misses generic form 'defineProps<Props>()'. Add: 'grep -E "defineProps(<|\\(" matching both. Or use: 'grep "defineProps" ignoring parentheses for broader match. Verify TypeScript syntax. Check Vue 3 Composition API version.
### Reactive/ref count inflated by counting imports and comments

grep matches 'import { ref }' and '// ref()' comments. Filter: 'grep -v "^\\s*//" | grep -v "import" | grep -c "ref("' excluding non-usage. Or limit: 'grep "<script setup>" -A 100' to script body only. Verify grep patterns. Check component structure.
### ESLint Vue plugin not found despite installation

Verify ESLint Vue plugin installation: 'npm list eslint-plugin-vue' or 'npm list @vue/eslint-config-typescript'. Install: 'npm install --save-dev eslint-plugin-vue @vue/eslint-config-typescript'. Check ESLint configuration. Verify Vue project setup.
### vue-tsc not found despite npm installation

Verify vue-tsc installation: 'npm list vue-tsc' or 'npx vue-tsc --version'. Install: 'npm install --save-dev vue-tsc'. Check PATH. Verify Node.js version compatibility. Ensure TypeScript is installed.
### Script setup detection fails with TypeScript lang attribute

Pattern '<script setup' misses TypeScript variant '<script setup lang="ts">'. Add: 'grep -E "<script setup( lang=\"ts\")?" matching both. Or use: 'grep "<script setup" ignoring lang attribute. Verify Vue component syntax. Check Vue 3 version.
