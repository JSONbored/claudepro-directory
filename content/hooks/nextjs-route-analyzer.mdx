...
category: hooks
description: >-
  Analyzes Next.js page routes and generates a route map when pages are added or
  modified. This PostToolUse hook provides comprehensive Next.js route analysis
  with comprehensive Next.js route analysis for both Pages and App Router
  including Pages Router detection (pages/, src/pages/), App Router detection
  (app/, src/app/), route file detection (page.tsx, page.jsx, route.tsx,
  route.jsx), and layout file detection (layout.tsx, layout.jsx), dynamic route
  detection with parameter mapping including bracket notation detection ([id],
  [slug]), catch-all route detection ([...slug]), optional catch-all route
  detection ([[...slug]]), parameter name extraction, and route parameter
  documentation, API endpoint discovery and documentation generation including
  API route detection (/api/ paths), route.tsx/jsx detection in App Router,
  pages/api/ detection in Pages Router, API endpoint listing, and API route
  documentation, route hierarchy visualization and structure analysis including
  route tree generation, parent-child route relationships, nested route
  detection, and route depth analysis, catch-all and optional catch-all route
  detection with catch-all pattern matching ([...slug]), optional catch-all
  pattern matching ([[...slug]]), catch-all route classification, and catch-all
  route recommendations, route groups and parallel routes analysis including
  route group detection ((group)), parallel route detection (@parallel), route
  group organization, and parallel route structure, static and dynamic route
  classification with static route detection (no brackets), dynamic route
  detection ([param]), route type classification, and route type statistics, and
  route map generation with export capabilities including JSON route map
  generation (nextjs-routes.json), route statistics (total, static, dynamic,
  API), route file mapping, and export format options. The hook automatically
  detects Next.js project structure (package.json with next dependency),
  identifies routing directories (pages/, app/, src/pages/, src/app/), analyzes
  route files (page.tsx, route.tsx, layout.tsx), classifies route types (static,
  dynamic, catch-all, API), generates route maps in JSON format, and provides
  comprehensive route analysis reports with optimization recommendations.
cardDescription: >-
  Analyzes Next.js page routes and generates a route map when pages are added or
  modified.
...
installable: true
installCommand: >-
  mkdir -p .claude/hooks && touch .claude/hooks/nextjs-route-analyzer.sh &&
  chmod +x .claude/hooks/nextjs-route-analyzer.sh
usageSnippet: >-
  mkdir -p .claude/hooks && touch .claude/hooks/nextjs-route-analyzer.sh &&
  chmod +x .claude/hooks/nextjs-route-analyzer.sh
copySnippet: |-
...
- Comprehensive Next.js route analysis for both Pages and App Router including Pages Router detection (pages/, src/pages/ directories), App Router detection (app/, src/app/ directories), route file detection (page.tsx, page.jsx, route.tsx, route.jsx for App Router, *.tsx, *.jsx for Pages Router), layout file detection (layout.tsx, layout.jsx) for App Router, and project structure validation (package.json with next dependency)
...
The hook processes all files in pages/ or app/ directories. For large projects, consider adding file type filtering (*.tsx, *.jsx only) or exclude non-route files like components and utilities. Verify file path patterns. Test with various file types.
