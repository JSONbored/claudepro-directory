...
description: >-
  ...
  (Rust). This hook automatically detects file types and applies appropriate
  formatting rules, ensuring consistent code style across team projects without
  manual intervention. Features include language-specific formatting rules,
  automatic formatter detection, configurable timeout and error handling,
  preservation of file permissions and structure, and silent operation with
  optional feedback. The hook supports multiple programming languages in the
  same project, automatically fixes formatting issues after AI code generation,
  and reduces code review friction by handling formatting automatically.
  Supports both traditional formatters (Prettier, Black) and modern
  high-performance alternatives (Ruff for Python, which is 10-100x faster than
  Black while maintaining Black-compatible formatting).
cardDescription: >-
  Automatically formats code files after Claude writes or edits them using
