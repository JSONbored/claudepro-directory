...
description: >-
  ...
  This hook runs on file write/edit operations and automatically identifies and
  executes tests related to changed files using dependency analysis, test file
  pattern matching, and framework-specific test discovery. Features include
  intelligent test selection based on code changes using Jest
  --findRelatedTests, Vitest test selection patterns, and pytest test discovery,
  parallel test execution for faster feedback using Jest workers, Vitest
  concurrent tests, and pytest-xdist, support for multiple testing frameworks
  including Jest v29.7.0+, Vitest v4.0.7+, pytest 9.0.0+, Go testing, Maven, and
  Gradle, fail-fast mode for quick feedback stopping on first failure, smart
  retry for flaky tests with configurable retry counts, impact analysis and
  dependency mapping to find tests affected by code changes, and integration
  with CI/CD pipelines for automated testing workflows.
cardDescription: >-
  Automatically run relevant tests when code changes are detected using
...
- Test file pattern matching to automatically detect test files (*.test.js, *.spec.js, test_*.py) and skip running tests when editing test files themselves
