...
description: >-
  ...
  during development. Supports Docker Engine 24.0+, Docker Compose v2 (docker
  compose), BuildKit for faster builds, multi-stage builds, layer caching
  optimization, and intelligent file detection for Docker-related
  configurations. Features include automatic Docker image rebuilding on
  Dockerfile changes with BuildKit support and layer caching, Docker Compose v2
  service rebuilding for compose file updates with dependency resolution,
  intelligent file detection for Docker-related configurations (Dockerfile,
  docker-compose.yml, .dockerignore, .dockerfile), support for multiple Docker
  file patterns and variations with explicit Dockerfile path specification,
  build status reporting and error handling with detailed build output and image
  information, development environment synchronization with automatic container
  updates, BuildKit integration for faster builds with cache mount support, and
  .dockerignore optimization for reduced build context size.
cardDescription: >-
  Automatically rebuilds Docker containers when Dockerfile or docker-compose.yml
  files are modified.
