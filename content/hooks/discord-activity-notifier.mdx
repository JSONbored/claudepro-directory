...
description: >-
  ...
  This Notification hook automatically sends rich embed messages to Discord
  webhooks when Claude Code activities occur, providing real-time team
  visibility into development workflows. Features include real-time Discord
  notifications for Claude Code activities (tool usage, file operations,
  errors), rich embed messages with file information, timestamps, Git
  branch/commit details, and file type icons, dynamic color coding based on
  action types (success: green, error: red, warning: yellow, edit/write:
  purple), team collaboration and activity visibility with configurable webhook
  integration, silent operation with fallback error handling and rate limiting
  support, Discord webhook API integration using curl with proper JSON
  formatting, and embed customization with fields, footers, and timestamps
  following Discord embed specifications.
cardDescription: Sends development activity updates to Discord channel for team collaboration.
...
installable: true
installCommand: >-
  mkdir -p .claude/hooks && touch .claude/hooks/discord-activity-notifier.sh &&
  chmod +x .claude/hooks/discord-activity-notifier.sh
usageSnippet: >-
  mkdir -p .claude/hooks && touch .claude/hooks/discord-activity-notifier.sh &&
  chmod +x .claude/hooks/discord-activity-notifier.sh
copySnippet: |-
