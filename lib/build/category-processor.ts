...
} from '@/lib/config/build-category-config';
import { logger } from '@/lib/logger';
import { generateSlugFromFilename } from '@/lib/schemas/content-generation.schema';
...
/**
 * Generate TypeScript file content from build results
 * Modern code generation with type safety
 *
 * @param categoryId - Category identifier
 * @param content - Content items
 * @param isMetadata - Whether this is metadata-only export
...
export function generateTypeScriptExport(
  ...
  const exportName = isMetadata ? `${categoryId}Metadata` : `${categoryId}Full`;
  const typeComment = isMetadata ? 'Metadata-only exports' : 'Full content exports';
  const typeName = exportName.charAt(0).toUpperCase() + exportName.slice(1);

  return `/**
 * Auto-generated ${typeComment} for ${categoryId}
...
export const ${exportName} = ${JSON.stringify(content, null, 2)} as const;

export type ${typeName}Type = typeof ${exportName};
`;
}
