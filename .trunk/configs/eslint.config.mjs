 ...
 *   (Prettier is handled by Trunk natively, not via ESLint plugin)
...
import architecturalRules from '../plugins/eslint-plugin-architectural-rules.js';
...
export default tseslint.config(
  ...
  },
  {
    ...
    plugins: {
      ...
      // Note: Prettier is handled by Trunk natively, not via ESLint plugin
    },
    settings: {
    ...
    rules: {
      ...
      // ============================================
      // Note: Prettier is now handled by Trunk natively as a separate linter
      // Trunk runs Prettier separately, so no ESLint integration needed
      ...
      // Disabled: Auto-fix conflicts with Prettier (Trunk runs Prettier separately)
      ...
      // ============================================
      // Note: import-x/order is auto-fixable
      // Prettier is handled by Trunk separately, so conflicts are minimized
      'import-x/order': [
        'error',
        {
       ...
       * @autofix DISABLED - Prettier handles semicolons correctly (via Trunk)
       * @note Prettier is handled by Trunk natively as a separate linter
       */
