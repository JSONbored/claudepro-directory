 ...
 * Paths updated to work from .trunk/configs/ location
 */
...
export default defineConfig({
  // Path relative to project root (__dirname is .trunk/configs/)
  ...
  reporter: [
    ...
    ['html', { outputFolder: '.trunk/test-results/html-report', open: 'never' }],
    [
      'json',
      {
        outputFile: '.trunk/test-results/results.json',
        // Enhanced JSON structure for better CI integration
      },
    ],
    // GitHub Actions reporter for CI
    process.env.CI ? ['github'] : ['list'],
    // JUnit reporter for Trunk Flaky Tests integration
    // Always enabled for Trunk cloud analysis (not just CI)
    [
      'junit',
      {
        outputFile: '.trunk/test-results/playwright/junit.xml',
        // Include full test names and suite information
  ...
  // Path relative to project root (__dirname is .trunk/configs/)
  globalSetup: path.resolve(__dirname, '../../config/tests/playwright-global-setup.ts'),
