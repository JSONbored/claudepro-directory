...
CREATE POLICY "service_role_only_job_runs" ON public.job_runs FOR ALL TO service_role USING (true)
WITH
  CHECK (true);

-- Deny all access to anon and authenticated roles
CREATE POLICY "deny_anon_authenticated_job_runs" ON public.job_runs FOR ALL TO anon,
...
CREATE POLICY "service_role_only_mfa_attempts" ON public.mfa_failed_verification_attempts FOR ALL TO service_role USING (true)
WITH
  CHECK (true);

-- Deny all access to anon and authenticated roles
CREATE POLICY "deny_anon_authenticated_mfa_attempts" ON public.mfa_failed_verification_attempts FOR ALL TO anon,
authenticated USING (false)
WITH
  CHECK (false);
...
-- This table contains sensitive authentication data
CREATE POLICY "service_role_only_password_attempts" ON public.password_failed_verification_attempts FOR ALL TO service_role USING (true)
WITH
  CHECK (true);

-- Deny all access to anon and authenticated roles
CREATE POLICY "deny_anon_authenticated_password_attempts" ON public.password_failed_verification_attempts FOR ALL TO anon,
authenticated USING (false)
WITH
  CHECK (false);
