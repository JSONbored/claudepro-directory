...
import type { GetRelatedContentInput } from '../../lib/types';
...
export async function handleGetRelatedContent(
  ...
  try {
    ...
    try {
      ...
      logger.error(
        { error: normalized, tool: 'getRelatedContent', args: rpcArgs },
    ...
    if (!data || data.length === 0) {
      logger.info(
        ...
        'getRelatedContent completed with no results'
      );
      return {
        content: [
          {
    ...
    logger.error(
      { error: normalized, tool: 'getRelatedContent', slug, category },
      'getRelatedContent tool error'
    );
    throw normalized;
  }
}
