...
import type { GetMcpServersInput } from '../../lib/types';
...
export async function handleGetMcpServers(
  ...
  try {
    ...
    if (!(result && result.items) || result.items.length === 0) {
      logger.info(
        { tool: 'getMcpServers', duration_ms: Date.now() - startTime, limit, resultCount: 0 },
        'getMcpServers completed with no results'
      );
      return {
        content: [
          {
    ...
    try {
      ...
      logger.warn(
        { error: normalized, tool: 'getMcpServers', slugs: slugs.slice(0, 10) },
   ...
   Stats: ${server.stats.views} views, ${server.stats.bookmarks} bookmarks`;
    ...
    logger.error({ error: normalized, tool: 'getMcpServers', limit }, 'getMcpServers tool error');
    throw normalized;
  }
}
