...
import type { GetContentByTagInput } from '../../lib/types';
...
export async function handleGetContentByTag(
  ...
  try {
    ...
    try {
      ...
      logger.error(
        { error: normalized, tool: 'getContentByTag', args: rpcArgs },
        'ContentService.getContentPaginated failed'
      );
      throw normalized;
    }
    ...
    if (items.length === 0) {
      ...
      logger.info(
        ...
        'getContentByTag completed with no results'
      );
      return {
        content: [
          {
    ...
    if (logic === 'AND') {
      ...
      if (filteredItems.length === 0) {
        ...
        logger.info(
          ...
          'getContentByTag completed with no results (AND logic)'
        );
        return {
          content: [
            {
    ...
    logger.error(
      { error: normalized, tool: 'getContentByTag', tags, logic, category },
