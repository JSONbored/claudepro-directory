...
import type { DownloadContentForPlatformInput } from '../../lib/types';
...
export async function handleDownloadContentForPlatform(
  ...
  try {
    ...
    logger.info(
      ...
      'downloadContentForPlatform completed successfully'
    );
    ...
    logger.error(
      { error: normalized, tool: 'downloadContentForPlatform', platform, category, slug },
      'downloadContentForPlatform tool error'
    );
    throw normalized;
  }
}
