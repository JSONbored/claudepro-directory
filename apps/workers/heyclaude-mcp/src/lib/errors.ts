...
export const ERROR_MESSAGES: Record<McpErrorCode, string> = {
  ...
  INVALID_PLATFORM:
    'Invalid platform specified. Supported platforms: claude-code, cursor, chatgpt-codex, generic.',
...
export const ERROR_RECOVERY: Partial<Record<McpErrorCode, string[]>> = {
  ...
  DATABASE_ERROR: ['The database may be temporarily unavailable', 'Try again in a few moments'],
};
...
export function createErrorResponse(
  ...
  const errorCode =
    typeof code === 'string'
      ? Object.values(McpErrorCode).includes(code as McpErrorCode)
