...
const Commands = () => {
  const [searchResults, setSearchResults] = useState<Command[]>(commands);
  ...
  const handleSearchResults = (results: Command[]) => {
    setSearchResults(results);
  };

  const processedCommands = sortItems(applyFilters(searchResults));

  const categories = [...new Set(commands.map(command => command.category))];
  const tags = [...new Set(commands.flatMap(command => command.tags))];
  const authors = [...new Set(commands.map(command => command.author))];
        ...
        <div className="container mx-auto px-4 py-20">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <p className="text-lg text-muted-foreground mb-8 leading-relaxed">
              Powerful commands to supercharge your Claude workflow. From code analysis to content generation, 
              discover commands that make complex tasks simple.
            </p>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="space-y-8">
          ...
          <SearchBar
            ...
            placeholder="Search commands by name, category, or functionality..."
          />
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div className="flex items-center gap-4">
              <span className="text-sm text-muted-foreground">
                {processedCommands.length} command{processedCommands.length !== 1 ? 's' : ''} found
              </span>
