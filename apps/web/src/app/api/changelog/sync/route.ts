...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Use atomic upsert RPC (checks if exists, inserts if not, returns existing if found)
    // SEO fields are omitted - the database trigger will auto-generate them when NULL
    const changelogData = await service.upsertChangelogEntry({
      p_version: version,
      p_date: date,
      p_slug: slug,
      p_tldr: tldrValue,
      p_what_changed: whatChanged ?? '',
      p_content: content,
      p_raw_content: rawContentValue,
      p_changes: changes,
      // SEO fields are omitted (optional) - RPC will pass NULL, trigger will auto-generate them
      p_metadata: {
        version,
      },
    });
