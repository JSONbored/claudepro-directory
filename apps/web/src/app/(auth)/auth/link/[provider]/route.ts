...
/**
 * Handles GET requests to initiate linking an OAuth provider to the authenticated account by redirecting the client to the appropriate next step.
 *
 * Validates the provider parameter, preserves an optional `next` return path, and:
 * - redirects to the connected accounts page with an error if the provider is invalid;
 ...
 * @param params - An object with a Promise-resolved `provider` string extracted from the route parameters.
