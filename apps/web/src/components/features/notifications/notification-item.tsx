...
export function NotificationItem({ notification }: NotificationItemProps) {
  ...
  return (
    <motion.div
      ...
      className="border-border/50 bg-card relative rounded-lg border p-4 shadow-sm transition-shadow duration-200 hover:shadow-md"
    >
      <button
        type="button"
        onClick={handleDismiss}
        className="text-muted-foreground hover:bg-accent hover:text-foreground focus:ring-accent absolute top-2 right-2 rounded-md p-1 transition-colors duration-150 focus:ring-2 focus:outline-none"
