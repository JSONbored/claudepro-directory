...
/**
 * Render the account sidebar for the authenticated user, including avatar, name/email, profile link, and navigation.
 *
 * Loads user settings (profile) and sponsorships in parallel where possible; if settings are missing the component
 * attempts to create/ensure a user record and reload settings. Failures are logged and safe UI defaults are used
 * (no profile image, empty sponsorships) when data cannot be loaded.
 *
 * @param user - Authenticated Supabase user for the current session
 * @param userNameMetadata - Fallback display name from authentication metadata when profile name is absent
 * @param userImageMetadata - Fallback image URL from authentication metadata when profile image is absent
 * @returns The account sidebar React element
 *
 * @see getUserSettings
