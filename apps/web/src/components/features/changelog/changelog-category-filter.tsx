...
import { size, padding, cluster, gap, justify, minWidth, height, marginLeft, width } from '@heyclaude/web-runtime/design-system';
...
export function CategoryFilter({ activeCategory, categoryCounts }: CategoryFilterProps) {
  return (
    <TabsList className={`grid ${width.full} ${gap.tight} lg:${width.auto} lg:auto-cols-fr lg:grid-flow-col`}>
      {FILTER_CATEGORIES.map((category) => (
        <TabsTrigger key={category} value={category} className={`${cluster.compact} ${size.sm}`}>
          ...
          <UnifiedBadge
            ...
            className={`${marginLeft.tight} ${height.buttonSm} ${minWidth.badge} ${justify.center} ${padding.xSnug} ${size.xs}`}
          >
