...
export const isBookmarkedAction = authedAction
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    const data = await fetchCached(
      ...
      {
        key: `${ctx.userId}-${parsedInput.content_type}-${parsedInput.content_slug}`,
...
export const isFollowingAction = authedAction
  ...
  .action(async ({ parsedInput, ctx }) => {
    const data = await fetchCached(
      (client) => new AccountService(client).isFollowing(ctx.userId, parsedInput.user_id),
      {
        key: `${ctx.userId}-${parsedInput.user_id}`,
...
export const getBookmarkStatusBatch = authedAction
  ...
  .action(async ({ parsedInput, ctx }) => {
    const data = await fetchCached(
      (client) => new AccountService(client).isBookmarkedBatch(ctx.userId, parsedInput.items),
      {
        key: `${ctx.userId}-${JSON.stringify(parsedInput.items)}`,
...
export const getFollowStatusBatch = authedAction
  ...
  .action(async ({ parsedInput, ctx }) => {
    const data = await fetchCached(
      (client) => new AccountService(client).isFollowingBatch(ctx.userId, parsedInput.user_ids),
      {
        key: `${ctx.userId}-${parsedInput.user_ids.sort().join('-')}`,
...
export const getActivitySummary = authedAction
  ...
  .action(async ({ ctx }) => {
    const data = await fetchCached(
      (client) => new AccountService(client).getUserActivitySummary(ctx.userId),
      {
        key: `${ctx.userId}-summary`,
...
export const getActivityTimeline = authedAction
  ...
  .action(async ({ parsedInput: { type, limit = 20, offset = 0 }, ctx }) => {
    const data = await fetchCached(
      (client) =>
        new AccountService(client).getUserActivityTimeline(ctx.userId, type, limit, offset),
      {
        key: `${ctx.userId}-${type ?? 'all'}-${limit}-${offset}`,
        tags: ['user-activity', `user-${ctx.userId}`],
        ttlKey: 'cache.user_activity.ttl_seconds',
        useAuth: true,
        fallback: {
...
export const getUserIdentities = authedAction
  ...
  .action(async ({ ctx }) => {
    const data = await fetchCached(
      (client) => new AccountService(client).getUserIdentities(ctx.userId),
      {
        key: `${ctx.userId}-identities`,
        tags: ['users', `user-${ctx.userId}`],
