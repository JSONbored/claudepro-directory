...
import { buildReadmeMarkdown } from '@heyclaude/edge-runtime/changelog/readme-builder.ts';
...
import { buildSecurityHeaders, logInfo } from '@heyclaude/shared-runtime';
...
async function handleSitewideReadme(logContext?: BaseLogContext): Promise<Response> {
  ...
  try {
    const readmeData = await service.getSitewideReadme();
...
async function handleCategoryLlmsTxt(category: ContentCategory): Promise<Response> {
  ...
  try {
    const data = await service.getCategoryLlmsTxt(category);
...
async function handleChangelogEntryLlmsTxt(slug: string): Promise<Response> {
  ...
  try {
    const data = await service.getChangelogEntryLlmsTxt(slug);
...
async function handleToolLlms(tool: string): Promise<Response> {
  ...
  try {
    const data = await service.getToolLlmsTxt(tool);
