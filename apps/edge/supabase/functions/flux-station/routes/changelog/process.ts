  ...
  try {
    ...
    for (const msg of messages) {
      ...
      if (!isValidChangelogWebhookProcessingJob(msg.message)) {
        const invalidMsgLogContext = {
          ...batchLogContext,
          msg_id: msg.msg_id.toString(),
          operation: 'validate-payload',
        };
        await logError('Invalid changelog webhook processing job structure', invalidMsgLogContext);

        // Delete invalid message to prevent infinite retries
        try {
          await pgmqDelete(CHANGELOG_PROCESSING_QUEUE, msg.msg_id);
        } catch (error) {
          await logError('Failed to delete invalid message', invalidMsgLogContext, error);
        }
