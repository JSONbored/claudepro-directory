...
import { createMockEnv } from '../__tests__/test-utils.ts';
...
describe('Environment Utilities', () => {
  ...
  describe('requireEnvVar', () => {
    ...
    it('should throw error when key is missing', () => {
      const env = createMockEnv();
      expect(() => requireEnvVar(env, 'REQUIRED_KEY')).toThrow(
    ...
    it('should throw custom error message when provided', () => {
      const env = createMockEnv();
      expect(() => requireEnvVar(env, 'REQUIRED_KEY', 'Custom error message')).toThrow(
