...
import type { GetCategoryConfigsInput } from '../../lib/types.js';
...
export async function handleGetCategoryConfigs(
  ...
  try {
    ...
    if (!data || !Array.isArray(data)) {
      ...
      logger.info('getCategoryConfigs completed with no results', {
    ...
    const duration = Date.now() - startTime;
    logger.info('getCategoryConfigs completed successfully', {
      tool: 'getCategoryConfigs',
      duration_ms: duration,
      category,
