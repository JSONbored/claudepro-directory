...
import type { SubscribeNewsletterInput } from '../../lib/types.js';
...
async function sendInngestEvent(
  ...
  // Get Inngest configuration from environment (runtime-agnostic)
...
export async function handleSubscribeNewsletter(
  ...
  try {
    ...
    logger.info('subscribeNewsletter completed successfully', {
      tool: 'subscribeNewsletter',
    ...
    logger.error('subscribeNewsletter tool error', normalized, {
      tool: 'subscribeNewsletter',
      email,
      source,
    });
