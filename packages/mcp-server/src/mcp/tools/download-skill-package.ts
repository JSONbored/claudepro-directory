/**
 * downloadSkillPackage Tool Handler
 *
 * Downloads Skills ZIP file from Supabase Storage.
...
/**
 * Downloads a Skills ZIP file from Supabase Storage.
 *
 ...
 * @throws If skill not found or download fails
 */
export async function handleDownloadSkillPackage(
  ...
  if (rootUri && context.elicit) {
    const confirmed = await context.elicit({
      type: 'boolean',
      description: `Download Skills package "${slug}" to ${rootUri}? This will write the ZIP file to your filesystem.`,
    });
    if (confirmed === false) {
  ...
  try {
    // Get download URL
    const downloadUrl = getStorageDownloadUrl('skills', slug, env);
    const fileName = getStorageFileName('skills', slug, 'zip');
    ...
    logger.info('downloadSkillPackage completed successfully', {
      tool: 'downloadSkillPackage',
      slug,
    ...
    return {
      ...
      _meta: {
        success: true,
        message: filePath
          ? `Skill package downloaded successfully to ${filePath}`
          : `Skill package download URL generated: ${downloadUrl}`,
        slug,
        fileName,
        downloadUrl,
        ...(filePath && { filePath }),
    ...
    const normalized = normalizeError(error, 'downloadSkillPackage tool failed');
    logger.error('downloadSkillPackage tool error', normalized, {
      tool: 'downloadSkillPackage',
      slug,
      rootUri: rootUri || null,
    });
