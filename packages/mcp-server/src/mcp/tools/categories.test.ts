...
describe('listCategories Tool Handler', () => {
  ...
  it('should return categories with counts', async () => {
    ...
    // Seed data for getCategoryConfigs (uses category_configs.findMany with Prismocker)
    if ('setData' in prismocker && typeof (prismocker as any).setData === 'function') {
      (prismocker as any).setData('category_configs', mockCategoryConfigsFromDb);
    }
    // Mock RPC call for getSearchFacets (uses $queryRawUnsafe)
  ...
  it('should return categories with zero counts when facets fail', async () => {
    ...
    // Mock RPC call for getSearchFacets - will fail
    queryRawUnsafeSpy.mockRejectedValue(new Error('Facets failed') as any);
  ...
  it('should throw error when category configs fail', async () => {
    ...
    const findManyMock = jest.fn().mockRejectedValue(new Error('Config fetch failed') as any);
  ...
  it('should throw error when no category data returned', async () => {
    // Seed empty data for getCategoryConfigs
    if ('setData' in prismocker && typeof (prismocker as any).setData === 'function') {
  ...
  it('should cache getCategoryConfigs results on duplicate calls (caching test)', async () => {
    // Seed data for getCategoryConfigs
    const mockCategoryConfigsFromDb = [
      {
        category: 'agents' as const,
    ...
    const result1 = await handleListCategories({}, context);
    const cacheAfterFirst = getRequestCache().getStats().size;

    // Second call - should use cache for getCategoryConfigs
