...
import type { GetPopularInput } from '../../lib/types.js';
...
export async function handleGetPopular(
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      logger.info('getPopular completed with no results', {
        tool: 'getPopular',
    ...
    const duration = Date.now() - startTime;
    logger.info('getPopular completed successfully', {
      tool: 'getPopular',
      duration_ms: duration,
      category,
    ...
    logger.error('getPopular tool error', normalized, { tool: 'getPopular', category });
    throw normalized;
  }
}
