...
import {
	...
	spacing,
	typography,
} from "@heyclaude/edge-runtime/utils/email/theme.ts";
							...
							{customFooter || (
								<Section style={footerStyle}>
									<Text style={footerTextStyle}>
										...
										<Link
											href={utmLink(baseUrl, "footer_home")}
									...
									<Text style={footerCopyrightStyle}>
										© {new Date().getFullYear()} ClaudePro Directory. All rights
										reserved.
									</Text>
								</Section>
							)}
...
export async function renderEmailTemplate<TProps>(
	...
	if (!props) {
		throw new Error(
			"renderEmailTemplate requires props when you pass a component reference.",
		);
	}

	// React.createElement needs the component type and props
	// Type assertion needed because React.createElement overloads don't match our generic ComponentType
	// We need to cast to any to satisfy React.createElement's overload constraints
	const Component = template as React.ComponentType<any>;
