...
async function generateActionFile(
  ...
  if (hasRevalidatePath || hasRevalidateTag) {
    ...
    if (hasRevalidatePath) nextImports.push('revalidatePath');
    if (hasRevalidateTag) nextImports.push('revalidateTag');
    imports.push(`import { ${nextImports.join(', ')} } from 'next/cache';`);
  }

  if (hasCacheInvalidation) {
    imports.push("import { nextInvalidateByKeys } from '../cache-tags';");
    imports.push("import { getCacheConfigSnapshot } from '../cache-config';");
  }

  let successHookCall = '';
  if (config.hooks?.onSuccess) {
    // Assume the hook file is relative to actions directory or an absolute import
    // For simplicity, we will assume it's a relative import from current file (in actions/) to hooks/
    // So if hooks.onSuccess is './hooks/job-hooks', it imports correctly.
    // We need to extract the function name from the import path? No, user provides path and we need to know export?
    // Let's assume the config provides the import path, and we assume a default export or named export matching the filename?
    // Better: config.hooks.onSuccess is the module path, and we assume a named export "onSuccess" or something.
    // Actually, let's make config.hooks.onSuccess be: { path: string, export: string }
    // Or simpler string: './hooks/job-hooks#onJobCreated'
