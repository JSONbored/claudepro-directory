...
import variableConsolidationPlugin, {
  transformations,
} from '../utils/css-migration/postcss-variable-consolidation.ts';
...
async function generateConsolidationReport(): Promise<ConsolidationReport> {
  ...
  for (const [value, vars] of duplicates.entries()) {
    if (vars.length > 1) {
      ...
      const recommended =
        newThemeVar ||
        vars.reduce((prev, curr) => (curr.name.length < prev.name.length ? curr : prev));
...
export async function applyVariableConsolidation(dryRun: boolean = false): Promise<void> {
  ...
  const reportPath = join(
    PROJECT_ROOT,
    '.cursor/tailwind-cleanup/consolidation-application-report.md'
  );
