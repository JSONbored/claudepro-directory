...
async function main() {
  try {
    ...
    console.log('⚠️  Looking for deprecated semantic utility usage (should migrate to Direct Tailwind)\n');
    
    const result = await designSystemPlugin.check();
    
    console.log('\n=== Design System Migration Report ===\n');
    console.log(`Files using deprecated semantic utilities: ${result.summary.filesUsingSemanticUtilities}`);
    ...
    if (result.deprecatedSemanticUtilities.length > 0) {
      ...
      console.log('These files import from @heyclaude/web-runtime/design-system (style utilities).');
    ...
    if (result.deprecatedSemanticUtilities.length === 0 && result.deprecatedOldConstants.length === 0) {
