...
export function getPlaceholderImageUrl(
      ...
      : typeof window !== 'undefined'
        ? (
            window as unknown as {
              __NEXT_DATA__?: { env?: { ['NEXT_PUBLIC_SUPABASE_URL']?: string } };
            }
          ).__NEXT_DATA__?.env?.['NEXT_PUBLIC_SUPABASE_URL']
        : undefined;
  ...
  if (!supabaseUrl) {
    ...
    const encoded = typeof btoa !== 'undefined' ? btoa(unescape(encodeURIComponent(svg))) : svg; // Fallback for Node.js (though this shouldn't happen in client context)
