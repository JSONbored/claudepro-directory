...
export function extractCodeBlocksFromMarkdown(markdown: string): ExtractedCodeBlock[] {
  ...
  for (let i = 0; i < codeBlocks.length - 1; i++) {
    ...
    const currentBlockEnd =
      markdown.indexOf('```', markdown.indexOf(currentBlock.code)) + currentBlock.code.length + 3;
  ...
  if (codeBlocks.length === 0) {
    ...
    const looksLikeCode =
      /^\s*(import|export|function|class|const|let|var|def|#!|<\?|package|public|private|interface|type)\s/.test(
        markdown.trim()
      );
