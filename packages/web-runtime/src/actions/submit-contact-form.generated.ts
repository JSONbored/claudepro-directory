...
import type { InsertContactSubmissionReturns } from '@heyclaude/database-types/postgres-types';
import { z } from 'zod';
...
export const submitContactForm = authedAction
  ...
  .action(async ({ parsedInput, ctx }) => {
    try {
      const rawResult = await runRpc<InsertContactSubmissionReturns>(
      ...
      const result = (Array.isArray(rawResult) ? rawResult[0] : rawResult) as InsertContactSubmissionReturns extends (infer U)[] ? U : InsertContactSubmissionReturns;
      ...
      if (result?.submission_id) {
        ...
        const hookResult = await onContactSubmission(
