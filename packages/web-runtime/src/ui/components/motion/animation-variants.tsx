 ...
 */
export function useFadeInVariants(options?: { y?: number }): Variants {
 ...
 * @returns Animation variants (transition should be passed separately to motion component)
 */
export function useScaleVariants(options?: { scale?: number }): Variants {
  const shouldReduceMotion = useReducedMotion();
  const { scale = 0.95 } = options || {};

  return {
    initial: shouldReduceMotion ? { opacity: 0 } : { opacity: 0, scale },
...
export function useSlideVariants(options?: {
  ...
  const shouldReduceMotion = useReducedMotion();
  const { direction = 'up', distance = 20 } = options || {};
  ...
  return {
    initial: shouldReduceMotion ? { opacity: 0 } : { opacity: 0, ...directionMap[direction] },
