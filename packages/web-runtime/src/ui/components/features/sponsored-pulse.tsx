...
import { trackSponsoredClick, trackSponsoredImpression } from '../../../actions/pulse.ts';
import { logUnhandledPromise, normalizeError } from '../../../entries/core.ts';
      ...
      (entries) => {
        ...
        if (entry?.isIntersecting && !impressionTracked.current) {
          ...
          trackSponsoredImpression({
            ...
            const normalized = normalizeError(error, 'SponsoredPulse: impression tracking failed');
            logUnhandledPromise('SponsoredPulse: impression failed', normalized, {
              component: 'SponsoredPulse',
              sponsoredId,
              position: position ?? 0,
            });
          });
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      ...
      trackSponsoredClick({
        ...
        const normalized = normalizeError(error, 'SponsoredPulse: click tracking failed');
        logUnhandledPromise('SponsoredPulse: click failed', normalized, {
          component: 'SponsoredPulse',
          sponsoredId,
        });
      });
