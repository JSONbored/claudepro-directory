...
import {
  newsletter_source as NewsletterSource,
  content_category as ContentCategory,
} from '@prisma/client';
import type { newsletter_sync_status } from '@prisma/client';
import { copy_type as CopyTypeEnum } from '@prisma/client';
...
export const subscribeNewsletter = createInngestFunction(
  ...
  async ({ event, step, logContext }: InngestHandlerContext) => {
    ...
    logger.info(
      ...
      'Newsletter subscription started'
    );
      ...
      async (): Promise<{
        ...
        try {
          ...
          logger.warn(
            ...
            'Resend sync failed, continuing with subscription'
          );
          return {
    ...
    const subscription = await step.run(
      ...
      async (): Promise<{
        ...
        const newsletterSourceValues = Object.values(
          NewsletterSource
        ) as readonly newsletter_source[];
        ...
        const isValidCopyType = (value: string | null | undefined): value is copy_type => {
          if (!value) return false;
        ...
        const finalSource: newsletter_source = isValidNewsletterSource(source) ? source : 'footer';
        ...
        if (referrer) {
          rpcArgs.p_referrer = referrer;
        }
        if (copyType && isValidCopyType(copyType)) {
          rpcArgs.p_copy_type = copyType;
        }
        if (copyCategory && isValidContentCategory(copyCategory)) {
          rpcArgs.p_copy_category = copyCategory;
        }
        if (copySlug) {
          rpcArgs.p_copy_slug = copySlug;
        }
        if (syncResult['resendContactId']) {
          rpcArgs.p_resend_contact_id = syncResult['resendContactId'] as string;
        }
        if (syncResult['syncError']) {
          rpcArgs.p_sync_error = syncResult['syncError'] as string;
        }
        if (
          syncResult['topicIds'] &&
          Array.isArray(syncResult['topicIds']) &&
          syncResult['topicIds'].length > 0
        ) {
    ...
    await step.run('invalidate-cache', async () => {
      try {
        ...
        logger.info({ ...logContext, tag: 'newsletter' }, 'Cache invalidated');
      } catch (error) {
    ...
    const welcomeEmailResult = await step.run(
      'send-welcome-email',
      async (): Promise<{
        sent: boolean;
        emailId: string | null;
    ...
    if (welcomeEmailResult.sent) {
      await step.run('enroll-onboarding', async () => {
        try {
          ...
          logger.info({ ...logContext, email: validatedEmail }, 'Enrolled in onboarding sequence');
        } catch (error) {
