...
# Claude MCP Server Development: Build Custom AI Integrations from Scratch
...
<Checklist type="testing"
  ...
  requirements={[
    { 
      item: "Claude Desktop installed (macOS, Windows, or Linux)",
      description: "Version 1.0+ with MCP support enabled",
      link: "https://claude.ai/download"
    },
    {
      item: "Node.js v18+ or Python 3.11+ environment",
      description: "TypeScript SDK v1.18.1 or Python MCP v1.2.0+",
      link: "https://nodejs.org"
    },
    {
      item: "Familiarity with JSON-RPC and async programming", 
      description: "Understanding of protocol-based communication",
      link: "#"
    },
    {
      item: "Access to Claude Desktop config file",
...
<Checklist type="testing"
  items={[
    { 
      test: "Unit test individual tool handlers",
      command: "npm test -- --coverage",
      expected: "100% coverage for tool logic, input validation verified"
    },
    {
      test: "Integration test transport layer",
      command: "npm run test:integration",
      expected: "All JSON-RPC methods respond correctly within 100ms"
    },
    {
      test: "Load test with concurrent connections",
      command: "artillery run load-test.yml",
      expected: "Handles 100 concurrent sessions maintaining <200ms response"
    },
    {
      test: "Security scan for vulnerabilities", 
      command: "npm audit && snyk test",
      expected: "No high/critical vulnerabilities in dependencies"
    },
    {
      test: "Validate Claude Desktop integration",
      command: "Check Claude Developer Tools",
      expected: "Server connected, all tools visible in Claude interface"
    }
  ]}
/>
...
<Accordion
  items={[
    {
      question: "Server fails to connect in Claude Desktop",
      answer: "Verify absolute paths in configuration file match exactly. Check stderr output using Developer Tools console. Ensure Node.js/Python executable paths are correct. Common issue: relative paths cause connection failures immediately."
    },
    {
      question: "Tools don't appear in Claude interface",
      answer: "Confirm server capabilities include 'tools: true' setting. Check tool registration happens before server.connect() call. Validate input schemas using Zod or Pydantic correctly. Inspector shows which tools register successfully."
    },
    {
      question: "Session state not persisting between calls",
      answer: "Implement external storage replacing in-memory objects. Redis provides simple session management starting quickly. Set appropriate TTLs preventing memory exhaustion. Session IDs must be unique per conversation."
    },
    {
      question: "Performance degrades with multiple users",
      answer: "Implement connection pooling for database queries. Add caching layers reducing redundant computations. Use streaming responses for long-running operations. Monitor memory usage preventing leaks accumulating."
    },
    {
      question: "Authentication tokens expire during sessions",
      answer: "Implement refresh token rotation automatically. Store tokens securely using platform keychains. Handle 401 responses triggering re-authentication flows. PKCE prevents token interception consistently."
    }
  ]}
/>
...
<RelatedResources
  ...
  resources={[
    {
      title: "Advanced MCP Security Patterns",
      description: "Implement enterprise-grade security for MCP servers. Master OAuth flows, token management, and audit logging in production environments.",
      url: "/guides/tutorials/mcp-security-patterns",
      type: "tutorial"
    },
    {
      title: "MCP Server Testing Strategies",
      description: "Comprehensive testing approaches for MCP servers. Learn unit, integration, and load testing techniques ensuring reliability.",
      url: "/guides/tutorials/mcp-testing-strategies",
      type: "tutorial"
    },
    {
      title: "Database MCP Server Implementation",
      description: "Build production database connectors with MCP. Connect Postgres, MongoDB, and MySQL with proper pooling and optimization.",
      url: "/guides/workflows/database-mcp-servers",
      type: "guide"
    },
    {
      title: "Enterprise MCP Deployment Case Study",
      description: "Learn how to deploy MCP at scale with custom server implementations.",
      url: "/guides/use-cases/enterprise-mcp-deployment",
      type: "example"
    },
    {
      title: "MCP Performance Troubleshooting",
      description: "Diagnose and fix common MCP performance issues. Memory leaks, slow responses, and connection problems solved.",
      url: "/guides/troubleshooting/mcp-performance-issues",
      type: "troubleshooting"
    },
    {
      title: "MCP Protocol Specification Deep Dive",
      description: "Understand MCP protocol internals completely. JSON-RPC implementation details and transport mechanism comparisons.",
      url: "/guides/tutorials/mcp-protocol-internals",
      type: "reference"
    }
  ]}
/>